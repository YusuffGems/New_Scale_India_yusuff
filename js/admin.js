...
function renderAdmin(page) {
  ...
  <div id="admin">
    <aside class="side" id="side">
      <div class="side-head">
        <img src="assets/images/logo.png" alt="${CONFIG.orgName}" class="side-logo">
...
function openBatchForm(editId) {
  ...
  /* Batch IDs already known — from existing batches and from any
     candidates loaded by bulk upload. Typing one fills the form. */
  const knownIds = [...new Set([
    ...DB.batches.map(x => x.batchId),
    ...DB.candidates.map(c => c.batchId)
  ])].filter(Boolean);

  modal(b ? t('edit') + ' — ' + b.batchId : t('createBatch'), `
    <label class="field">
      <span>${t('batchId')} — type or pick, the rest fills itself</span>
      <input class="input mono" id="nbId" list="nbIdList" autocomplete="off"
             value="${esc(v('batchId'))}" ${b ? 'readonly' : 'placeholder="3882781-4"'}
             oninput="batchIdLookup()" onchange="batchIdLookup()">
      <datalist id="nbIdList">${knownIds.map(x => `<option value="${esc(x)}">`).join('')}</datalist>
    </label>
    <div id="nbNotice"></div>
    ...
    `<button class="btn ghost" onclick="closeModal()">${t('cancel')}</button>
     <button class="btn ghost" id="nbUpload" onclick="uploadForCurrentBatch()">⤒ ${t('bulkUpload')}</button>
     <button class="btn" onclick="saveBatch()">${t('save')}</button>`, true);

  batchIdLookup();
}

/* Fills every field from the batch — or, if the batch does not
   exist yet, from candidates already loaded under that Batch ID.
   The candidate list renders under the form either way. */
function batchIdLookup() {
  const id = ($('#nbId').value || '').trim();
  const notice = $('#nbNotice');
  const set = (sel, val) => { const el = $(sel); if (el && val !== undefined && val !== null && val !== '') el.value = val; };
  ...
  const b = getBatch(id);
  const cands = DB.candidates.filter(c => c.batchId === id).sort((a, c) => a.sno - c.sno);

  if (b) {
    set('#nbType', b.batchType); set('#nbQp', b.qpCode); set('#nbScheme', b.scheme);
    set('#nbPartner', b.partner); set('#nbCentre', b.centreName); set('#nbAddr', b.centreAddress);
    set('#nbDist', b.district); set('#nbState', b.state); set('#nbAssessor', b.assessorId);
    set('#nbDate', b.assessmentDate); set('#nbStart', b.startTime); set('#nbEnd', b.endTime);
    set('#nbStatus', b.status);

    notice.innerHTML = `<div class="card" style="padding:11px;margin:-4px 0 14px;border-color:var(--blue-300)">
      <div class="row wrap">
        <span class="pill blue">Existing batch — fields filled</span>
        <span class="pill grey mono">${b.assessmentKey}</span>
        ${b.isLocked ? '<span class="pill green">🔒 Locked</span>' : ''}
      </div>
      <div class="muted" style="font-size:12px;margin-top:6px">Saving updates this batch instead of creating a new one.</div></div>`;
  } else if (cands.length) {
    /* Batch ID seen in uploaded candidate rows but no batch record yet. */
    const c = cands[0];
    set('#nbType', c.batchType); set('#nbQp', c.qpCode);
    set('#nbCentre', c.centreName); set('#nbAddr', c.centreAddress);
    notice.innerHTML = `<div class="card" style="padding:11px;margin:-4px 0 14px;border-color:var(--saffron)">
      <span class="pill amber">${cands.length} uploaded candidate(s) found</span>
      <div class="muted" style="font-size:12px;margin-top:6px">QP code and centre filled from the upload. Complete the rest and save.</div></div>`;
  } else {
    notice.innerHTML = `<div class="muted" style="font-size:12px;margin:-6px 0 14px">
      New Batch ID — key will be <b class="mono">LSSC-${esc(id)}</b></div>`;
  }

  drawBatchFormCandidates(id, cands);
}

function drawBatchFormCandidates(id, cands) {
  const box = $('#nbCands');
  if (!box) return;

  if (!cands.length) {
    box.innerHTML = `<div class="card" style="padding:16px;margin-top:6px;text-align:center">
      <b style="font-size:13.5px">No candidates in this batch yet</b>
      <div class="muted" style="font-size:12.5px;margin:5px 0 10px">
        Save the batch first, then upload the roster — or upload now and the rows attach to this Batch ID.</div>
      <button class="btn ghost" onclick="uploadForCurrentBatch()">⤒ ${t('bulkUpload')}</button></div>`;
    return;
  }

  box.innerHTML = `
    <div class="spread" style="margin:8px 0 8px">
      <span class="eyebrow">${t('candidateList')} — ${cands.length}</span>
      <button class="btn ghost" style="padding:6px 11px;font-size:12px" onclick="uploadForCurrentBatch()">⤒ ${t('add')}</button>
    </div>
    <div class="tbl-scroll" style="border:1px solid var(--line);border-radius:10px;max-height:240px;overflow-y:auto">
      <table><thead><tr><th>${t('slNo')}</th><th>${t('candidateId')}</th><th>${t('candidateName')}</th>
        <th>${t('mobile')}</th><th>${t('qpCode')}</th></tr></thead><tbody>
        ${cands.map(c => `<tr>
          <td>${c.sno}</td>
          <td class="mono" style="font-size:11.5px">${c.candidateId}</td>
          <td>${esc(c.name)}</td>
          <td class="mono" style="font-size:11.5px">${esc(c.mobile)}</td>
          <td class="mono" style="font-size:11.5px">${esc(c.qpCode)}</td></tr>`).join('')}
      </tbody></table></div>`;
}

function uploadForCurrentBatch() {
  const id = ($('#nbId') ? $('#nbId').value : '').trim();
  if (!id) return toast('Enter the Batch ID first.', 'err');
  if (!getBatch(id)) return toast('Save the batch first, then upload its candidates.', 'err');
  closeModal();
  openImporter(id);
}
...
function saveBatch() {
  ...
  if (existing && existing.isLocked) return toast('Locked assessments cannot be edited.', 'err');
  ...
  if (existing) {
    ...
    /* Keep candidates in step with the batch they belong to. */
    batchCandidates(id).forEach(c => {
      c.batchType = fields.batchType; c.qpCode = fields.qpCode; c.qpName = fields.qpName;
      c.centreName = fields.centreName; c.centreAddress = fields.centreAddress;
    });
    saveStore(); audit('BATCH_UPDATED', id);
    closeModal(); renderAdmin('batches');
  ...
  /* Rows uploaded before the batch existed now inherit its details. */
  const orphans = DB.candidates.filter(c => c.batchId === id);
  orphans.forEach((c, i) => {
    c.sno = i + 1; c.batchType = fields.batchType;
    c.qpCode = fields.qpCode; c.qpName = fields.qpName;
    c.centreName = fields.centreName; c.centreAddress = fields.centreAddress;
  });

  saveStore(); audit('BATCH_CREATED', id + (orphans.length ? ` (${orphans.length} candidates attached)` : ''));
  closeModal(); renderAdmin('batches');
  toast('Batch created. Key: LSSC-' + id, 'ok');
  if (!orphans.length) offerCandidateUpload(id);
}
